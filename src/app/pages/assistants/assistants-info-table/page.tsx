...
import AssistantPopup from "@/app/pages/assistants/assistant-popup/page";
...
export function AssistantsTable() {
  ...
  useEffect(() => {
    const fetchAssistants = async () => {
      try {
        const response = await axios.get('http://localhost:4000/api/assistants');
  ...
  return (
    <div className="rounded-[10px] border border-stroke bg-white p-4 shadow-1 dark:border-dark-3 dark:bg-gray-dark dark:shadow-card sm:p-7.5">
      <Table>
        <TableHeader>
          <TableRow className="border-none bg-[#F7F9FC] dark:bg-dark-2 [&>th]:py-4 [&>th]:text-base [&>th]:text-dark [&>th]:dark:text-white">
            <TableHead className="min-w-[155px] xl:pl-7.5">Assistant Name</TableHead>
      ...
      {selectedFile && <AssistantPopup file={selectedFile} onClose={() => setSelectedFile(null)} />}
    </div>
  );
}
