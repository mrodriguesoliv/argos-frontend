...
export function PeriodPicker<TItem extends string>({
  ...
  return (
    <Dropdown isOpen={isOpen} setIsOpen={setIsOpen}>
      ...
      <DropdownContent
        align="end"
        className="min-w-[7rem] overflow-hidden rounded-lg border border-[#E8E8E8] bg-white p-1 font-medium text-black shadow-md data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0 data-[state=closed]:zoom-out-95 data-[state=open]:zoom-in-95 data-[side=bottom]:slide-in-from-top-2 data-[side=left]:slide-in-from-right-2 data-[side=right]:slide-in-from-left-2 data-[side=top]:slide-in-from-bottom-2 dark:border-dark-3 dark:bg-dark-2 dark:text-current"
      >
        <ul>
          {(items || ["monthly", "yearly"]).map((item) => (
            <li key={crypto.randomUUID()}>
              <button
                className="flex w-full select-none items-center truncate rounded-md px-3 py-2 text-sm capitalize outline-none hover:bg-[#F9FAFB] hover:text-white dark:hover:bg-[#FFFFFF1A] dark:hover:text-white"
                onClick={() => {
